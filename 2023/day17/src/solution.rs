...
pub fn solve<const TURN_MIN: i32, const STRAIGHT_MAX: i32>(input: &str) -> i32 {
    ...
    while let Some((mut d, (x, y), c, dir)) = q.pop() {
        ...
        let f = mv(x,y,dir);
        if c >= TURN_MIN && bck(r) && !v[(right(dir) as i32 + 4*(1 + 11*(r.0 + w*r.1))) as usize] { q.push(d, (d, r, 1, right(dir))); }
        if c >= TURN_MIN && bck(l) && !v[(left(dir) as i32 + 4*(1 + 11*(l.0 + w*l.1))) as usize] { q.push(d, (d, l, 1, left(dir))); }
        if c < STRAIGHT_MAX && bck(f) && !v[(dir as i32 + 4*(c+1 + 11*(f.0 + w*f.1))) as usize] { q.push(d, (d, f, c+1, dir)); }
    }
